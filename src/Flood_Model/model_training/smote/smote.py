"""
SMOTE + Logistic Regression Model - Train, Save, and Evaluate
This script applies SMOTE for class imbalance handling, trains a Logistic Regression model,
and evaluates it with comprehensive metrics.
"""

import pandas as pd
import numpy as np
import joblib
import matplotlib.pyplot as plt
import warnings

warnings.filterwarnings("ignore")

from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import (
    precision_score,
    recall_score,
    f1_score,
    roc_auc_score,
    accuracy_score,
    roc_curve,
    confusion_matrix,
    classification_report,
)
from imblearn.over_sampling import SMOTE


def load_and_prepare_data():
    """Load CSV data and prepare features for modeling"""
    print("Loading data...")
    path = r"C:\Users\ps302\OneDrive\Desktop\Hydrology\src\Flood_Model\data\proceessed\flood_training_data_10k_clean.csv"
    df = pd.read_csv(path)

    print(f"Data shape: {df.shape}")
    print(f"\nFirst few rows:\n{df.head()}")

    # Check class distribution before SMOTE
    print(f"\nOriginal class distribution:")
    print(df["flood"].value_counts())

    # Feature division
    X = df.drop(["flood", "point_id", "easting", "northing"], axis=1)
    y = df["flood"]

    return X, y, df


def apply_smote(X, y):
    """Apply SMOTE for class imbalance handling"""
    print("\nApplying SMOTE for class imbalance handling...")
    smote = SMOTE(random_state=42)
    X_resampled, y_resampled = smote.fit_resample(X, y)

    print(f"✓ SMOTE applied")
    print(f"Original dataset size: {X.shape[0]}")
    print(f"Resampled dataset size: {X_resampled.shape[0]}")
    print(f"\nResampled class distribution:")
    unique, counts = np.unique(y_resampled, return_counts=True)
    for u, c in zip(unique, counts):
        print(f"  Class {u}: {c}")

    return X_resampled, y_resampled


def plot_balanced_spatial_distribution(df, y):
    """Plot balanced flood vs non-flood spatial distribution using SMOTE on geographic features"""
    print("\nGenerating balanced spatial distribution plot...")

    # Apply SMOTE separately to geographic features for visualization (following notebook cell 14)
    X_geo = df[["easting", "northing"]]
    y_geo = df["flood"]

    smote_geo = SMOTE(random_state=42)
    X_geo_resampled, y_geo_resampled = smote_geo.fit_resample(X_geo, y_geo)

    df_geo_resampled = pd.DataFrame(X_geo_resampled, columns=["easting", "northing"])
    df_geo_resampled["flood"] = y_geo_resampled

    plt.figure(figsize=(8, 6))
    plt.scatter(
        df_geo_resampled["easting"],
        df_geo_resampled["northing"],
        c=df_geo_resampled["flood"],
        cmap="coolwarm",
        s=5,
    )
    plt.xlabel("Easting", fontsize=12)
    plt.ylabel("Northing", fontsize=12)
    plt.title(
        "Flood vs Non-Flood Spatial Distribution (After SMOTE)",
        fontsize=14,
        fontweight="bold",
    )
    plt.colorbar(label="Flood (0=No, 1=Yes)")
    plt.tight_layout()
    plt.show()

    print("\nResampled spatial class counts:")
    print(df_geo_resampled["flood"].value_counts().sort_index())
    print("✓ Balanced spatial distribution plot displayed")


def split_data(X_resampled, y_resampled):
    """Split resampled data into train and test sets"""
    print("\nSplitting resampled data...")
    X_train, X_test, y_train, y_test = train_test_split(
        X_resampled, y_resampled, test_size=0.2, random_state=42, stratify=y_resampled
    )

    print(f"Training set: {X_train.shape}")
    print(f"Test set: {X_test.shape}")

    return X_train, X_test, y_train, y_test


def train_model(X_train, y_train):
    """Train Logistic Regression model with balanced class weights"""
    print("\nTraining Logistic Regression model with SMOTE...")

    model = LogisticRegression(
        class_weight="balanced", random_state=42, max_iter=1000, solver="liblinear"
    )

    model.fit(X_train, y_train)
    print("✓ Model trained successfully")

    return model


def save_model(model, save_path):
    """Save trained model to pickle file"""
    print(f"\nSaving model to {save_path}...")
    joblib.dump(model, save_path)
    print(f"✓ Model saved successfully")


def make_predictions(model, X_test):
    """Generate predictions on test data"""
    print("\nGenerating predictions...")
    y_pred = model.predict(X_test)
    y_pred_proba = model.predict_proba(X_test)[:, 1]

    print(f"✓ Predictions generated (shape: {y_pred.shape})")
    return y_pred, y_pred_proba


def calculate_metrics(y_test, y_pred, y_pred_proba):
    """Calculate classification metrics"""
    print("\n" + "=" * 50)
    print("TEST SET PERFORMANCE METRICS")
    print("=" * 50)

    precision = precision_score(y_test, y_pred)
    recall = recall_score(y_test, y_pred)
    accuracy = accuracy_score(y_test, y_pred)
    f1 = f1_score(y_test, y_pred)
    roc_auc = roc_auc_score(y_test, y_pred_proba)
    cm = confusion_matrix(y_test, y_pred)

    print(f"Precision:     {precision:.4f}")
    print(f"Recall:        {recall:.4f}")
    print(f"F1 Score:      {f1:.4f}")
    print(f"Accuracy:      {accuracy:.4f}")
    print(f"ROC AUC:       {roc_auc:.4f}")
    print(f"\nConfusion Matrix:")
    print(cm)

    print("\n" + "=" * 50)
    print("CLASSIFICATION REPORT")
    print("=" * 50)
    print(classification_report(y_test, y_pred))

    return {
        "precision": precision,
        "recall": recall,
        "accuracy": accuracy,
        "f1": f1,
        "roc_auc": roc_auc,
        "confusion_matrix": cm,
    }


def plot_roc_curve(y_test, y_pred_proba, roc_auc):
    """Plot ROC curve"""
    print("\nGenerating ROC curve...")
    fpr, tpr, _ = roc_curve(y_test, y_pred_proba)

    plt.figure(figsize=(10, 8))
    plt.plot(
        fpr,
        tpr,
        color="darkorange",
        lw=2,
        label=f"SMOTE + Logistic Regression (AUC = {roc_auc:.4f})",
    )
    plt.plot(
        [0, 1], [0, 1], color="navy", lw=2, linestyle="--", label="Random Classifier"
    )
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel("False Positive Rate (1 - Specificity)", fontsize=12)
    plt.ylabel("True Positive Rate (Recall)", fontsize=12)
    plt.title(
        "Receiver Operating Characteristic (ROC) Curve", fontsize=14, fontweight="bold"
    )
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    print("✓ ROC curve displayed")


def plot_confusion_matrix_heatmap(cm):
    """Plot confusion matrix as heatmap"""
    print("\nGenerating confusion matrix heatmap...")

    plt.figure(figsize=(8, 6))
    plt.imshow(cm, interpolation="nearest", cmap=plt.cm.Blues)
    plt.title(
        "Confusion Matrix - SMOTE + Logistic Regression", fontsize=14, fontweight="bold"
    )
    plt.colorbar()

    tick_marks = np.arange(2)
    plt.xticks(tick_marks, ["No Flood", "Flood"], fontsize=11)
    plt.yticks(tick_marks, ["No Flood", "Flood"], fontsize=11)

    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            plt.text(
                j,
                i,
                str(cm[i, j]),
                fontsize=14,
                ha="center",
                va="center",
                color="white" if cm[i, j] > cm.max() / 2 else "black",
            )

    plt.ylabel("True Label", fontsize=12)
    plt.xlabel("Predicted Label", fontsize=12)
    plt.tight_layout()
    plt.show()
    print("✓ Confusion matrix heatmap displayed")


def print_model_parameters(model):
    """Print model parameters"""
    print("\n" + "=" * 50)
    print("MODEL PARAMETERS")
    print("=" * 50)
    params = model.get_params()
    for key, value in params.items():
        print(f"{key}: {value}")


def main():
    """Main execution pipeline"""
    print("=" * 50)
    print("SMOTE + LOGISTIC REGRESSION - TRAINING & EVALUATION PIPELINE")
    print("=" * 50)

    # Load and prepare data
    X, y, df = load_and_prepare_data()

    # Apply SMOTE for class imbalance
    X_resampled, y_resampled = apply_smote(X, y)

    # Plot balanced spatial distribution (pass original df for geographic features)
    plot_balanced_spatial_distribution(df, y)

    # Split data
    X_train, X_test, y_train, y_test = split_data(X_resampled, y_resampled)

    # Train model
    model = train_model(X_train, y_train)

    # Print model parameters
    print_model_parameters(model)

    # Save model
    model_path = r"C:\Users\ps302\OneDrive\Desktop\Hydrology\src\Flood_Model\model_training\smote\best_smote_model.pkl"
    save_model(model, model_path)

    # Make predictions
    y_pred, y_pred_proba = make_predictions(model, X_test)

    # Calculate and display metrics
    metrics = calculate_metrics(y_test, y_pred, y_pred_proba)

    # Plot ROC curve
    plot_roc_curve(y_test, y_pred_proba, metrics["roc_auc"])

    # Plot confusion matrix
    plot_confusion_matrix_heatmap(metrics["confusion_matrix"])

    print("\n" + "=" * 50)
    print(" TRAINING AND EVALUATION COMPLETE")
    print("=" * 50)


if __name__ == "__main__":
    main()
